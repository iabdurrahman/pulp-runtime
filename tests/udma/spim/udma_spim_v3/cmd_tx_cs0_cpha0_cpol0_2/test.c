...
int main(void)
{
	...
	udma_spim_command[1] = SPI_CMD_SETUP_UCA(/* TX: 1; RX: 0 */ 1, /* word per transfer */ 1, spi_tx_data_0); /* transfer from spi_tx_data_0 address */
	...
	udma_spim_command[4] = SPI_CMD_SEND_CMD((uint16_t) 0x4b, 8, SPI_CMD_QPI_DIS); /* command = 0x4b, (maximum 16-bit), number of bit = 8, qspi = false */
	...
	/* configuration */
	udma_spim_command[0] = SPI_CMD_CFG(SPI_CMD_CFG_CLKDIV(4), SPI_CMD_CFG_CPOL_POS, SPI_CMD_CFG_CPHA_STD); /* spi configuration */
	udma_spim_command[1] = SPI_CMD_SETUP_UCA(/* TX: 1; RX: 0 */ 1, /* word per transfer */ 1, spi_tx_data_1); /* transfer from spi_tx_data_0 address */
	udma_spim_command[2] = SPI_CMD_SETUP_UCS(/* TX: 1; RX: 0 */ 1, /* word per transfer */ 1, 15); /* byte length - 1 */

	/* actual transfer */
	udma_spim_command[3] = SPI_CMD_SOT(SPI_CMD_SOT_CS0); /* argument 0 for csn[0], argument n is for csn[n] */
	udma_spim_command[4] = SPI_CMD_SEND_CMD((uint16_t) 0x4c, 8, SPI_CMD_QPI_DIS); /* data = 0x4c, (maximum 16-bit), number of bit = 8, qspi = false */
	udma_spim_command[5] = SPI_CMD_DUMMY(16); /* add dummy 16 bit */
	udma_spim_command[6] = SPI_CMD_TX_DATA(16, SPI_CMD_1_WORD_PER_TRANSF, SPI_CMD_DATA_WITDH(8), SPI_CMD_QPI_DIS, SPI_CMD_MSB_FIRST); /* transfer */
	/*udma_spim_command[7] = SPI_CMD_EOT(SPI_CMD_EOT_EVENT_ENA, SPI_CMD_EOT_GEN_EVT_OFFSET);*/ /* generate event, don't keep cs */
	udma_spim_command[7] = SPI_CMD_EOT(SPI_CMD_EOT_EVENT_DIS, SPI_CMD_EOT_GEN_EVT_OFFSET); /* don't generate event, don't keep cs */
